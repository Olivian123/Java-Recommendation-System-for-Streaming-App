...
public class ProiectPOO {
    ...
    public static void main(String[] args) {
        ...
        streamerReader.extractFromCSV("src/main/resources/" + args[0]);

        ArrayList<Stream> streams = new ArrayList<>();
        StreamReader streamReader = new StreamReader(streams);
        streamReader.extractFromCSV("src/main/resources/" + args[1]);

        ArrayList<User> users = new ArrayList<>();
        UserReader userReader = new UserReader(users);
        userReader.extractFromCSV("src/main/resources/" + args[2]);

        Scanner input;
        try {
            input = new Scanner(new File("src/main/resources/" + args[3]));
